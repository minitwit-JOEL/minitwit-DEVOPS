...
docker run -d --network=minitwit-network -p 8080:8081 --name=minitwit-api -e "ConnectionStrings__DefaultConnection=Host=minitwit-db;Port=5432;Database=minitwit;Username=postgres;Password=postgres" lukan707/minitwit-joel-api:latest
docker run -d --network=minitwit-network -p 3000:3000 --name=minitwit-web lukan707/minitwit-joel-web:latest 
